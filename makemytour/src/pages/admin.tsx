...
export default function AdminDashboard() {
  ...
  const getBaseUrl = () => { return process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"; };
  ...
  const loadInventory = async () => {
    ...
    try {
      const hotelRes = await axios.get(`${getBaseUrl()}/hotel`);
  ...
  const handleAddHotel = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post(`${getBaseUrl()}/admin/hotel`, {
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* 🚀 Tabs restore ho gaye dono side switch karne ke liye */}
